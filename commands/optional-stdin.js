...
function cat (stdin, filenames, done) {
  if (stdin && !filenames) return done(stdin); // `return` stops fn; could use else instead.
  filenames = filenames.split(' ');
  const texts = []; // const means no reassignment, but obj can be mutated
  var count = 0;
